    ...
    if method == "get":
        ...
        if test_name == "create branch":
            ...
            #           'rb') as fp:
                ...
                # params['files'] = files
            data = {
                'uploaded_file': (BytesIO(b'my file contents'), 'test_file.txt'),# we use StringIO to simulate file object
                'commit_message': 'kuk'
            }
        ...
        if test_name == "edit file":
            ...
            with open(os.path.join(os.getcwd(), 'files', 'test_file_99d4c5aa-4a57-4e76-9962-e38ea5a54895.txt'),
                      'rb') as fp:
                files = {'uploaded_file': fp}
                response = APP.test_client().put(endpoint, files=files, data=params)
